...
public:
    static rd::opt_or_wrapper<T> read(SerializationCtx const &ctx, Buffer const &buffer) {
        return buffer.readNullable<T>([&]() -> T { return S::read(ctx, buffer); });
    }

    static void write(SerializationCtx const &ctx, Buffer const &buffer, rd::opt_or_wrapper<T> const &value) {
...
public:
    static rd::Wrapper<T> read(SerializationCtx const &ctx, Buffer const &buffer) {
