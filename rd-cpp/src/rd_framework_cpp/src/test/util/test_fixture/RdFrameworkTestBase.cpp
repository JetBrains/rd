...
RdFrameworkTestBase::RdFrameworkTestBase()
	: clientLifetimeDef(Lifetime::Eternal())
	...
	, serverLifetime(serverLifetimeDef.lifetime)
{
	...
	clientProtocol = std::unique_ptr<IProtocol>(
		std::make_unique<Protocol>(/*serializers, */ clientIdentities, &clientScheduler, clientWire, clientLifetime));
	serverProtocol = std::unique_ptr<IProtocol>(
		std::make_unique<Protocol>(/*serializers,*/ serverIdentities, &serverScheduler, serverWire, serverLifetime));
