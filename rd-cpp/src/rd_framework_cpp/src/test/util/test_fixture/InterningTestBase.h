...
protected:
	...
	int64_t get_written_bytes_count(IProtocol* protocol, T& sender_model_view, int32_t offset, const std::wstring& suffix)
	{
		return measureBytes(protocol, [&]() {
			for_each(
				[&](int32_t const& k, std::wstring const& v) { sender_model_view.get_issues().set(k + offset, W(v + suffix)); });
		});
	}
	...
	void doTest(bool firstClient, bool secondClient, bool thenSwitchSides = false)
	{
		...
		auto first_bytes_written = get_written_bytes_count<String>(firstSenderProtocol, first_sender_model_view, 0, L"");
