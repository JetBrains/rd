...
namespace rd {
	namespace util {
			...
			try {
				...
				auto task1 = std::async([&] {
					messaging("Server to client",
							  proxyServer, proxyClient, serverToClientBuffer, serverToClientLifetime);
				}).share();

				auto task2 = std::async([&] {
					messaging("Client to server",
							  proxyClient, proxyServer, clientToServerBuffer, clientToServerLifetime);
				}).share();
		...
		void SocketProxy::messaging(const std::string &id, CSimpleSocket &source, CSimpleSocket &destination,
									Buffer &buffer, const SequentialLifetimes &lifetimes) const {
			while (!lifetime->is_terminated()) {
				try {
					int32_t length = source.Receive(DefaultBufferSize, buffer.data());
					if (length == -1) {
						throw std::runtime_error(id + ": error occurred while receive"
													  ", reason: " + source.DescribeError());
					}
					if (length == 0) {
						throw std::runtime_error(id + ": connection lost on receive"
													  ", reason: " + source.DescribeError());
					}
					...
					if (!lifetimes.is_terminated()) {
						...
						while (total_sent < length) {
							...
							if (sent == -1) {
								throw std::runtime_error(id + ": error occurred while send"
															  ", reason: " + destination.DescribeError());
							}
							if (sent == 0) {
								throw std::runtime_error(id + ": connection lost on send"
															  ", reason: " + destination.DescribeError());
							}
							total_sent += sent;
						...
						logger.info(
								id + ": message of length %d was not transferred, because lifetime was terminated",
								length);
					}
				}
				catch (std::exception const &e) {
		...
		SocketProxy::SocketProxy(std::string id, Lifetime lifetime, int serverPort) :
				id(std::move(id)), lifetime(lifetime), serverPort(serverPort),
		...
		SocketProxy::SocketProxy(const std::string &id, Lifetime lifetime, IProtocol const *protocol) :
				SocketProxy(std::move(id), std::move(lifetime),
							dynamic_cast<SocketWire::Server const *>(protocol->get_wire())->port) {}
		...
		void SocketProxy::start() {
			try {
				...
				if (!proxyServer->Initialize()) {
					throw std::runtime_error(id + ": initialize failed"
												  ", reason: " + proxyServer->DescribeError());
				}
				proxyServer->DisableNagleAlgoritm();
				if (!proxyServer->Open("127.0.0.1", serverPort)) {
					throw std::runtime_error(id + ": open failed"
												  ", reason: " + proxyServer->DescribeError());
				}
				...
				if (!proxyClient->Initialize()) {
					throw std::runtime_error(id + ": initialize failed"
												  ", reason: " + proxyClient->DescribeError());
				}
				if (!proxyClient->Listen("127.0.0.1", 0)) {
					throw std::runtime_error(id + ": listen failed"
												  ", reason: " + proxyClient->DescribeError());
				}

				port = proxyClient->GetServerPort();
				logger.info("Proxies for server on port %d and client on port %d created successfully",
							serverPort, port);

				auto thread = std::make_shared<std::thread>([this] {
					CActiveSocket *accepted_client = proxyClient->Accept();
					if (accepted_client == nullptr) {
						throw std::runtime_error(id + ": accept returns nullptr"
													  ", reason:  " + proxyClient->DescribeError());
					}
