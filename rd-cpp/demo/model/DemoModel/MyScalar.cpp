...
//primary ctor
MyScalar::MyScalar(bool sign_, signed char byte_, short short_, int32_t int_, int64_t long_): rd::IPolymorphicSerializable(), sign_(std::move(sign_)), byte_(std::move(byte_)), short_(std::move(short_)), int_(std::move(int_)), long_(std::move(long_)) { initialize(); }
//reader
MyScalar MyScalar::read(rd::SerializationCtx const& _ctx, rd::Buffer const & buffer)
{
    auto const &ctx = _ctx;
...
//writer
void MyScalar::write(rd::SerializationCtx const& _ctx, rd::Buffer const& buffer) const
{
    auto const &ctx = _ctx;
...
//equals trait
bool MyScalar::equals(rd::IPolymorphicSerializable const& object) const
{
