...
class RdExtTest : RdTestBase(false) {
    ...
    fun testExtension() {
        ...
        clientEntity.getOrCreateExtension("ext", DynamicExt::class.java) { DynamicExt("Ext!", "client") }
        clientWire.processAllMessages()
        //client send READY

        val serverExt = serverEntity.getOrCreateExtension("ext", DynamicExt::class.java) { DynamicExt("", "server") }
        serverWire.processAllMessages()
        //server send READY
