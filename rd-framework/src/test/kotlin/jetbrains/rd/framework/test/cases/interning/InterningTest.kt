...
class InterningTest: RdTestBase() {
    ...
    fun doTest(firstClient: Boolean, secondClient: Boolean, thenSwitchSides: Boolean = false) {
        val serverProperty = RdProperty(InterningTestModel).static(1).slave()
        val clientProperty = RdProperty(InterningTestModel).static(1)
        ...
        val clientModel = clientProperty.value
    ...
    fun testNestedInternedObjects() {
        val serverPropertyHolder = PropertyHolderWithInternRoot(RdProperty(InterningNestedTestModel.interned()).static(1).slave(), serverProtocol.serializationContext.withInternRootHere(true))
        val clientPropertyHolder = PropertyHolderWithInternRoot(RdProperty(InterningNestedTestModel.interned()).static(1), clientProtocol.serializationContext.withInternRootHere(false))
        ...
        val firstSendBytes = measureBytes(serverProtocol) {
            ...
            assertEquals(testValue, clientProperty.value, "Received value should be the same as sent one")
        }

        val secondSendBytes = measureBytes(serverProtocol) {
            serverProperty.value = testValue.inner!!
            assertEquals(testValue.inner, clientProperty.value, "Received value should be the same as sent one")
        }
