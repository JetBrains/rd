...
val<T> IReadonlyProperty<T>.hasValue : Boolean get() = maybe.hasValue
val<T> IReadonlyProperty<T?>.valueOrNull : T? get() = maybe.asNullable
...
fun <TSource, TResult> Iterable<IReadonlyProperty<TSource>>.fold(lifetime: Lifetime, initial: TResult, func: (TResult, TSource) -> TResult): Property<TResult> {
    ...
    for (p in this) {
        p.advise(lifetime) {
            val value = this.fold(initial) { acc, x -> if (x.hasValue) func(acc, x()) else acc }
            property.set(value)
        }
    }
    return property
...
fun <T> IReadonlyProperty<T>.flowInto(lifetime: Lifetime, target: IProperty<T>)
{
    advise(lifetime) { target.value = it }
}
...
fun <T : Any> IReadonlyProperty<T?>.adviseNotNullOnce(lifetime: Lifetime, handler: (T) -> Unit) = Lifetime.define(lifetime) { def, lt ->
