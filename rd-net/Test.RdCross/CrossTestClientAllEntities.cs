...
{
    public class CrossTestClientAllEntities : CrossTestClientBase
    {
        ...
        void AdviseAll(Lifetime lifetime, DemoModel model, ExtModel extModel)
        {
        ...
        public override void Run(string[] args)
        {
            ...
            SingleThreadScheduler.RunInCurrentStackframe(SocketLifetime, "", scheduler =>
            {
                var client = new SocketWire.Client(ModelLifetime, scheduler, Port, "DemoServer");
                
                Protocol = new Protocol("Server", new Serializers(), new Identities(IdKind.Server), scheduler,
                    client, SocketLifetime, new SerializationCtx());
        ...
        private static void CheckConstants()
        {
            ...
            Assert.Equals(ConstUtil.const_enum, MyEnum.@default);
            Assert.Equals(ConstUtil.const_string, "const_string_value");
            Assert.Equals(Base.const_base, 'B');
        }
    }
}