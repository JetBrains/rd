...
abstract class Declaration(open val pointcut: BindableDeclaration?) {
    ...
    //for customization
    private val settings = HashMap<IGeneratorProperty<*>, Any>()
    fun <T:Any> setting(key: IGeneratorProperty<T>, value: T) {
        settings[key] = value
    }
    fun setting(key: IGeneratorProperty<Unit>) = setting(key, Unit)

    @Suppress("UNCHECKED_CAST")
    fun <T:Any> getSetting(key: IGeneratorProperty<T>) : T? = (settings[key] as? T?) ?: pointcut?.getSetting(key)
...
sealed class Class(override val _name: String, override val pointcut : Toplevel, override val base: Abstract?) : BindableDeclaration(pointcut), INonNullableBindable {
...
abstract class Ext(pointcut : BindableDeclaration) : Toplevel(pointcut) {
    override val cl_name = "singleton"
    init {
        @Suppress("LeakingThis")
        root.singletons.add(this)
    }
}
...
abstract class Root(vararg val generators: IGenerator) : Toplevel(null) {
    ...
    val toplevels : List<Toplevel> get() = listOf(this) + singletons
