...
class RdGen : Kli() {
    ...
    fun run() : Boolean {
        ...
        sources.value?.also {
            if (!it.toFile().isDirectory) errorAndExit("Sources are incorrect. No folder found at '$it'")
        }?:println("Folder 'source' isn't specified, trying to find models in given classpath")

        val classloader = sources.value?.let { compileDsl(it) } ?: defaultClassloader
        if (error != null) return errorAndExit()
...
fun generateRdModel(classLoader: ClassLoader, namespacePrefixes: Array<String>
    ...
    data class GenPair(val generator: IGenerator, val root: Root) : Comparable<GenPair> {
        override fun compareTo(other: GenPair): Int {
            generator.folder.absolutePath.compareTo(other.generator.folder.absolutePath).let { if (it != 0) return it}
    ...
    toplevels.filterIsInstance<Root>().forEach { root ->
    ...
    for ((gen, root) in generatorsToInvoke) {
        val shouldClear = clearOutputFolderIfExists &&
            ...
            generatorsToInvoke.asSequence()
            .takeWhile { it.generator != gen }
            .none { gen.folder.absolutePath.startsWith(it.generator.folder.absolutePath) }


        //Here is the real part
        if (verbose) println("Invoke $gen on root $root, clearFolder=$shouldClear")
